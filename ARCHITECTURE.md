...
- **Four attachments now have to track the scene's rectangle.** The
  ...
  Centralizing it turns four places to forget into one call — and Slice 19 then
...
Slice 19** and needed no such thing: it cost one resource, one `PassKind`, and one
...
- **Every attachment must track the scene's rectangle.** Depth and colour
  ...
  without a special case. Since Slice 19 that rectangle is the scene's, not the
...
- ~~**A composite into an arbitrary rect**~~ — **landed as Slice 19**, driven by
